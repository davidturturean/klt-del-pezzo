...
/-!
# Submodules of actual sheaves of modules

Reviewed port of `Mathlib/Algebra/Category/ModuleCat/Sheaf/Submodule.lean`
at commit `5aedf732b6987e8c26ab3c9ebc855314f82b045f` (Apache-2.0).
The source SHA256 is
`f2e69aa5a46e25f2a31555645d86ca0aca115bfaa55c31d6f4e4625fe548897b`.

The sectionwise submodules must be stable under restriction and membership
must be local for the actual Grothendieck topology. From this proved local
condition, the existing pinned `Subpresheaf.isSheaf_iff` constructs the
actual sheaf of modules. The compatibility changes are the pinned names
`Subpresheaf`, `toPresheaf`, `AddCommGrp`, and `Sheaf.val`. No Cartier
locality, local triviality, or invertibility assertion is supplied here.
-/
...
/-- A stable family of actual submodules whose membership condition is local. -/
...
/-- The sheaf of modules supplied by the proved local membership condition. -/
def toSheafOfModules : SheafOfModules.{v} R where
  val := N.toPresheafOfModules
  isSheaf := by
    suffices Presieve.IsSheaf J N.toSubfunctor.toPresheaf by
      apply Presheaf.isSheaf_of_isSheaf_comp J (s := CategoryTheory.forget AddCommGrp)
      rwa [isSheaf_iff_isSheaf_of_type]
    rw [N.toSubfunctor.isSheaf_iff]
    · exact fun _ s hs ↦ N.isSheaf s hs
    · rw [← isSheaf_iff_isSheaf_of_type]
      exact GrothendieckTopology.HasSheafCompose.isSheaf _ M.isSheaf

/-- Inclusion of the constructed sheaf into the original module sheaf. -/
def ι : N.toSheafOfModules ⟶ M := ⟨N.toSubmodule.ι⟩
...
lemma le_iff {N₁ N₂ : M.Submodule} : N₁ ≤ N₂ ↔ N₁.toSubmodule ≤ N₂.toSubmodule := Iff.rfl

instance : InfSet M.Submodule where
  sInf s :=
    { toSubmodule := sInf ((·.toSubmodule) '' s)
      isSheaf X x hx := by
        simp only [PresheafOfModules.Submodule.sInf_obj, _root_.Submodule.mem_iInf]
        rintro _ ⟨N', hN', rfl⟩
...
lemma toSubmodule_sInf (s : Set M.Submodule) :
    (sInf s).toSubmodule = sInf ((·.toSubmodule) '' s) := rfl
...
lemma toSubmodule_inf (N₁ N₂ : M.Submodule) :
    (N₁ ⊓ N₂).toSubmodule = N₁.toSubmodule ⊓ N₂.toSubmodule := rfl
