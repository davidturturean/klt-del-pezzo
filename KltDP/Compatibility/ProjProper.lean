...
Released under Apache 2.0 license; see
  audit/library_reuse/projective_properness/mathlib_original_proof/LICENSE.
Authors: Patience Ablett, Kevin Buzzard, Harald Carlens, Wayne Ng Kwing King,
  Michael Schlößer, Justus Springer, Andrew Yang, Jujian Zhang

Bounded port of Mathlib commit 568f63d367aed3eca2b5efc21f2ac6c9daf18819,
AlgebraicGeometry/ProjectiveSpectrum/Proper.lean, finite-type and valuative
sections. Existing separatedness and local finite-type proofs are imported. The missing
homogeneous-cover lemma is imported from our separately reviewed source port.
The one-line localization lemma below is from the same upstream commit.
-/
...
/-!
# Properness of a finitely generated graded Proj

The proof retains `Algebra.FiniteType (𝒜 0) A` throughout. It proves local finite type
on the homogeneous affine charts, quasi-compactness by a finite generating cover, and universal closedness by the valuative criterion.
No properness hypothesis is introduced into the scheme or grading data.
-/

noncomputable section

namespace KltDP.Compatibility.ProjProper

open AlgebraicGeometry AlgebraicGeometry.Proj Scheme CategoryTheory Limits pullback
  HomogeneousLocalization

/-- A localization fraction with equal numerator and denominator equals one. -/
@[simp]
lemma localization_mk_self_mk {M : Type*} [CommMonoid M] {S : Submonoid M}
    (a : M) (haS : a ∈ S) : Localization.mk a ⟨a, haS⟩ = 1 :=
  Localization.mk_self ⟨a, haS⟩
...
instance proj_locallyOfFiniteType [Algebra.FiniteType (𝒜 0) A] :
    LocallyOfFiniteType (Proj.toSpecZero 𝒜) := by
  -- The already proved chart argument works with independent universes for R and A.
...
instance proj_quasiCompact [Algebra.FiniteType (𝒜 0) A] : QuasiCompact (Proj.toSpecZero 𝒜) := by
  classical
  ...
  have := congr($(KltDP.ProjGeneratedCover.iSup_basicOpen_eq_top 𝒜
...
lemma valuativeCriterion_existence [Algebra.FiniteType (𝒜 0) A] :
  ...
  rw [← KltDP.ProjGeneratedCover.iSup_basicOpen_eq_top 𝒜 (ι := x) (↑) (fun i ↦ ⟨_, hxd _ i.2⟩) (by simpa using hx),
...
instance proj_universallyClosed [Algebra.FiniteType (𝒜 0) A] : UniversallyClosed (Proj.toSpecZero 𝒜) := by
  rw [UniversallyClosed.eq_valuativeCriterion]
  exact ⟨valuativeCriterion_existence 𝒜, inferInstance⟩

end UniversallyClosed

instance proj_isProper [Algebra.FiniteType (𝒜 0) A] : IsProper (Proj.toSpecZero 𝒜) where
  toIsSeparated := inferInstance
  toUniversallyClosed := proj_universallyClosed 𝒜
  toLocallyOfFiniteType := proj_locallyOfFiniteType 𝒜

end KltDP.Compatibility.ProjProper
