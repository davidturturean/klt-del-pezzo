...
theorem transition_preserves_zero
    {X : TopCat.{u}} {J' : Type u} [SmallCategory J']
    (Y' : J' ⥤ (Opens X)ᵒᵖ ⥤ AddCommGrp.{u})
    {j₀ j j' : J'} (f : j₀ ⟶ j) (g : j ⟶ j')
    {U V : Opens X} (φ : U ⟶ V)
...
lemma transition_preserves_compat
  ...
  intro W φ c₀
  simp only [Functor.map_comp, NatTrans.comp_app, AddCommGrp.hom_comp,
...
theorem sheaf_section_zero_of_zero_on_cover
    {X : TopCat.{u}} {F : (Opens X)ᵒᵖ ⥤ AddCommGrp.{u}} (hF : TopCat.Presheaf.IsSheaf F)
...
theorem filtered_colimit_kills_all_restrictions
    ...
    {X : TopCat.{u}} (Y' : J' ⥤ (Opens X)ᵒᵖ ⥤ AddCommGrp.{u})
...
theorem filtered_colimit_restriction_eventually_zero_of_zero
  ...
  let ev V := (CategoryTheory.evaluation (Opens X)ᵒᵖ AddCommGrp.{u}).obj (op V)
  have hcU : IsColimit ((ev U).mapCocone c) := isColimitOfPreserves (ev U) hc
  have hcTyp := isColimitOfPreserves (CategoryTheory.forget AddCommGrp) hcU
  ...
  have h0 : ((CategoryTheory.forget AddCommGrp).mapCocone
      ((ev U).mapCocone c)).ι.app j₀
      (ConcreteCategory.hom ((Y'.obj j₀).map φ.op) b₀) =
    ((CategoryTheory.forget AddCommGrp).mapCocone
  ...
  obtain ⟨j₁, g₁, hg₁⟩ : ∃ (j₁ : J') (g₁ : j₀ ⟶ j₁),
    ...
    choose j_pair f_pair hf_pair using fun p : ↥t × ↥t ↦ show ∃ j' f, compatAfter j' f p from by
      ...
      obtain ⟨j', f, hf⟩ := ((Types.FilteredColimit.isColimit_eq_iff'
        (isColimitOfPreserves (CategoryTheory.forget AddCommGrp) (hcV Wkl)))
      ...
      have hf0 : ConcreteCategory.hom ((P.map f).app (op Wkl)) left0 =
        ...
        change
          ConcreteCategory.hom (((P ⋙ ev Wkl) ⋙ forget AddCommGrp).map f) left0 =
            ConcreteCategory.hom (((P ⋙ ev Wkl) ⋙ forget AddCommGrp).map f) right0
