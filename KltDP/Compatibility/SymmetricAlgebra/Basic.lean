...
-/
/-
Bounded source port from official Mathlib commit a5582bc4f33d6297becad2645a28a9e64fd51a7b,
Mathlib/LinearAlgebra/SymmetricAlgebra/Basic.lean.
Project-owned namespacing and this provenance note are adapted. The inherited
elab_as_elim attribute is omitted to preserve the project source gate.
The original quotient and mathematical proofs are retained.
Full upstream license: reuse_search/sources/mathlib_symmetric_initial/LICENSE.txt
in the codex-stage-projectivity-20260912 source dossier.
-/
...
quotiented out by. -/
inductive TensorSymRel : TensorAlgebra R M → TensorAlgebra R M → Prop where
  | mul_comm (x y : M) : TensorSymRel
...
abbrev SymmetricAlgebra := RingQuot (TensorSymRel R M)

namespace SymmetricAlgebra

/-- Algebra homomorphism from the tensor algebra over `M` to the symmetric algebra over `M`. -/
abbrev algHom : TensorAlgebra R M →ₐ[R] SymmetricAlgebra R M := RingQuot.mkAlgHom R (TensorSymRel R M)
...
instance : CommSemiring (SymmetricAlgebra R M) where
  mul_comm a b := by
    ...
    | ι x => induction a using SymmetricAlgebra.induction with
      ...
      | ι y => simp [commute_iff_eq, ι, ← map_mul, RingQuot.mkAlgHom_rel _ (TensorSymRel.mul_comm x y)]
...
instance (R M) [CommRing R] [AddCommMonoid M] [Module R M] : CommRing (SymmetricAlgebra R M) where
  ...
  __ := inferInstanceAs (Ring (RingQuot (TensorSymRel R M)))
...
def lift : (M →ₗ[R] A) ≃ (SymmetricAlgebra R M →ₐ[R] A) := by
  let equiv : (TensorAlgebra R M →ₐ[R] A) ≃
    {f : TensorAlgebra R M →ₐ[R] A // ∀ {x y}, (TensorSymRel R M) x y → f x = f y} := by
