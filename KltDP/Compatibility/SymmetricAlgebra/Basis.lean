...
Project-owned namespacing, the local import, and this provenance note are adapted.
The inherited global Cardinal.lift printing attribute and unused simps! generator
are omitted to preserve the project source gate. The original quotient and
mathematical proofs are retained; no generated repr_apply lemma is claimed.
...
noncomputable def _root_.KltDP.Basis.symmetricAlgebra (b : Basis κ R M) :
...
instance instModuleFree [Module.Free R M] : Module.Free R (SymmetricAlgebra R M) :=
  ...
  .of_basis (KltDP.Basis.symmetricAlgebra b)
