...
/-!
# One actual ramification blowdown has its exact rank drop and entire doubled forest
...
Picard bound and exact rank drop are all derived on that same actual target surface.
-/
...
local instance originalUnbranchedForestRankDropSeparated : S.toScheme.IsSeparated := surfaceSeparated S
local instance originalUnbranchedForestRankDropMonoidal : MonoidalCategory S.toScheme.Modules :=
...
local instance originalUnbranchedForestRankDropComponentsFinite : Finite (Components π N hbir) := by
...
local instance originalUnbranchedForestRankDropComponentsFintype : Fintype (Components π N hbir) :=
  Fintype.ofFinite _
...
theorem exists_ramification_blowdowns_with_forest_rank_drop
    ...
    ∃ (V : NormalProjectiveSurface k)
      ...
      (Ramification).card = N.card ∧
      (CoverSurface).picardRank = V.picardRank + N.card ∧ IsPointBlowupSequence CoverSurface V b ∧
  ...
  obtain ⟨V, hV, b, Q, hrank, hseq, hcontract, hQ, hmatrix⟩ :=
    KltDP.Geometry.exists_blowdowns_with_surviving_family_and_rank CoverSurface CoverRegular P hm hp
      (fun i : Bool × Components π N hbir => copy i.1 i.2) ha
  have hramcard : Nat.card {P : (CoverSurface).PrimeCurve // P ∈ Ramification} = N.card := by
    rw [Nat.card_eq_fintype_card, Fintype.card_coe, hcard]
  have hdrop : (CoverSurface).picardRank = V.picardRank + N.card := by
    simpa only [hramcard] using hrank
  ...
  refine ⟨V, hV, b, Q, hcard, hdrop, hseq, fun j hj => hcontract ⟨j, hj⟩, hQ, ?_, hpos, hLI, ?_, ?_, ?_⟩
  · intro i j
    calc
...
#print axioms KltDP.Geometry.UnbranchedExceptionalBlocks.exists_ramification_blowdowns_with_forest_rank_drop
