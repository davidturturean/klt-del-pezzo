...
/-!
# The same blowdowns preserve a surviving family and its exact rank drop
...
The exact Picard-rank drop and entire original intersection matrix are preserved. The
...
theorem exists_blowdowns_with_surviving_family_and_rank
    ...
    ∃ (T : NormalProjectiveSurface k)
      ...
      (Q' : κ → T.PrimeCurve),
      S.picardRank = T.picardRank + Nat.card ι ∧ IsPointBlowupSequence S T b ∧ (∀ i, IsExceptionalCurve b (E i)) ∧
  ...
  classical
  obtain ⟨T, hT, b, hrank, hseq, hcontract, hpreserve, hintersection⟩ :=
    exists_contract_disjoint_family_intersections_with_rank (GeneralResolution.contraction k)
      S hS E hminus hpair
  choose Q' hQ' using fun j => hpreserve (Q j) (havoid j)
  refine ⟨T, hT, b, Q', hrank, hseq, hcontract, hQ', ?_⟩
  funext i j
...
#print axioms KltDP.Geometry.exists_blowdowns_with_surviving_family_and_rank
