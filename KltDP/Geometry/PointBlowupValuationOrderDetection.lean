...
/-!
# An actual smooth resolution detects the original divisorial order
...
with exactly the original order on every unit of K(X). The comparison uses
...
theorem exists_pointBlowup_resolution_detecting_order
  ...
  refine ⟨S, π, hπ, D, hsmooth, ⟨b, hbR, hseq, hfac⟩, ?_⟩
  intro a
...
#check @KltDP.Geometry.exists_pointBlowup_resolution_detecting_order
#print axioms KltDP.Geometry.exists_pointBlowup_resolution_detecting_order
