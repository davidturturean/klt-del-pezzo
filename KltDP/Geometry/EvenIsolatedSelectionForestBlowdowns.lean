...
/-!
# The original even isolated selection constructs the complete forest blowdown
...
and span, and the strict target Picard bound are derived. No cover, branch
...
local instance evenIsolatedSelectionForestSeparated : S.toScheme.IsSeparated := surfaceSeparated S
local instance evenIsolatedSelectionForestMonoidal : MonoidalCategory S.toScheme.Modules :=
...
theorem exists_evenSelection_forest_blowdown
    (hneN : N.Nonempty)
  ...
  dsimp only
  obtain ⟨V, hV, b, Q, hcard, hseq, hcontract, hQ, hmatrix, hpos, hLI, hcount, hspan, hbound⟩ :=
    exists_ramification_blowdowns_with_forest_picard_bound π N hbir hmin hklt
      E hE L e h2 hred hne hIJ hiso hN hweil hself
  exact ⟨V, hV, b, Q, hcard, hseq, hcontract, hQ, hmatrix, hcount, hspan, hbound⟩

end KltDP.Geometry.UnbranchedExceptionalBlocks

#print axioms KltDP.Geometry.UnbranchedExceptionalBlocks.exists_evenSelection_forest_blowdown
