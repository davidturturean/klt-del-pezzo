...
/-!
The actual projective normal factor with geometrically connected fibers contracts
...
/-- The actual projective normal proper ample factor contracts precisely the original
labelled null primes, retaining both geometric and point-fiber connectedness. -/
theorem exists_projective_normal_factor_with_label_criterion (hn : 2 < n) :
    letI : IsIntegral (multiSurface (q + 1) n a) := multiSurface_isIntegral (q + 1) n a ha
    ∃ m : ℕ, 0 < m ∧ ∃ (Y : Scheme.{u}) (σ : Y ⟶ Spec (CommRingCat.of k)),
      IsProjectiveOverField σ ∧ IsProper σ ∧ IsNormalScheme Y ∧ ∃ hY : IsIntegral Y,
  ...
  letI : IsIntegral (multiSurface (q + 1) n a) := multiSurface_isIntegral (q + 1) n a ha
  obtain ⟨m, hm, Y, σ, hprojective, hσ, hnormal, hY, π, hπ, hproper, hsurj, hbir, hc,
      hgeom, hpoints, A, hA, hpower, hC⟩ :=
    PrimeCurveImageContraction.exists_projective_normal_factor_with_prime_criterion
  ...
  letI : IsIntegral Y := hY
  refine ⟨m, hm, Y, σ, hprojective, hσ, hnormal, hY, π, hπ, hproper, hsurj, hbir, hc,
