...
/-!
# Finite blowdowns preserve the full original intersection matrix
...
theorem exists_contract_disjoint_fin_intersections
  ...
  | zero =>
      ...
      refine ⟨S, hS, 𝟙 S.toScheme, .of_isIso _ inferInstance (by simp), ?_, ?_, ?_⟩
      · intro i
        exact Fin.elim0 i
      · intro D hD
  ...
  | succ n ih =>
      ...
      obtain ⟨T, hT, g, hseq, hcontract, hpreserve, hpairing⟩ :=
        ih S' hb.regular C' hminus' hpair'
      obtain ⟨z, hz, hzero, hdim, hbl⟩ := hb.center
      refine ⟨T, hT, b ≫ g, .step b g z hbl hseq, ?_, ?_, ?_⟩
      · intro i
...
theorem exists_contract_disjoint_family_intersections
    {k : Type u} [Field k] [IsAlgClosed k]
  ...
  obtain ⟨T, hT, f, hseq, hcontract, hpreserve, hpairing⟩ :=
    exists_contract_disjoint_fin_intersections hCa (Fintype.card ι) S hS (fun j => C (e.symm j))
      (fun j => hminus (e.symm j)) hp
  refine ⟨T, hT, f, hseq, ?_, ?_, ?_⟩
  · intro i
...
#print axioms KltDP.Geometry.exists_contract_disjoint_family_intersections
