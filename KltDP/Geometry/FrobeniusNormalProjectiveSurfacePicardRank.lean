...
/-!
# The original normal projective Frobenius contraction has numerical Picard rank one

Choose the existing full original contraction witness once. Its proved
all-prime criterion, connected fibres, ample target line and positive
pullback exponent discharge every input to the actual numerical-rank
producer. All original existential data and geometric conclusions are
retained, with numerical finite dimensionality and rank one appended.
-/
...
/-- The same original full contraction has a finite-dimensional numerical
Picard space of rank one, with no additional construction premise. -/
theorem exists_normal_projective_surface_rank_one (hn : 2 < n) :
      ...
      ∃ A : InvertibleSheaf S.toScheme, AmpleSerre.IsAmple A ∧
        ...
        ∃ U : S.toScheme.Opens,
          ...
          Nat.card (π.base '' KltDP.Geometry.exceptionalLocus
            (S := sourceSurface q n a ha (originalMultiStructureProjective k (q + 1) n a))
            (X := S) π) = 2 * n + 1 ∧
          S.NumericalSpaceFiniteDimensional ∧ S.picardRank = 1 := by
  letI : IsIntegral (multiSurface (q + 1) n a) :=
    multiSurface_isIntegral (q + 1) n a ha
  obtain ⟨m, hm, S, π, hπ, hproper, hsurj, hbir, hc, hgeom, hpoints,
    ...
    exists_normal_projective_surface_exact_exceptional_locus q n a ha hn
  letI : IsProper π := hproper
  letI : Surjective π := hsurj
  letI : IsIso π.c := hc
  have hrank := FrobeniusMultiCentreTargetPicardRank.picardRank_one
